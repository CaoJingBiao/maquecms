...
class Tag extends TagLib{
    ...
    /**
     * 这是一个闭合标签的简单演示  {tag:close time='time()'/}
     */
    ...
    public function tagOpen($tag, $content)
    {
        ...
        return '<?php $test_arr=[[1,3,5,7,9],[2,4,6,8,10]];print_r($test_arr);?>';

        //echo highlight_string($parse);exit;
        /*$parse = '<?php echo "abd"; ?>;';
        return $parse;*/
    ...
    */
    public function tagUedit($tag){
        ...
        if(!empty($imagePath)){
            $parse .= 'echo "<script type=\"text/javascript\"> um = UM.getEditor(\''.$id.'\',{imageUrl:\''.$imageUrl.'\',imagePath:'.$imagePath.'});</script>";';
        }else{

            //开始oss
            if(Config::get('type.type')=='OSS'){
            
                $parse .= 'echo "<script type=\"text/javascript\"> um = UM.getEditor(\''.$id.'\',{imageUrl:\''.$imageUrl.'\',imagePath:\"\"});</script>";';

            }else{
                //默认
                $parse .= 'echo "<script type=\"text/javascript\"> um = UM.getEditor(\''.$id.'\',{imageUrl:\''.$imageUrl.'\'});</script>";';
            }
