...
class News extends Common{
	...
	public function catelist(){
		...
		if($this->request->isGet()){
			...
			if($this->request->isajax()){
				...
				$list = $NewsCate -> GetCateListData($pid);
	...
	public function cateadd(){
		...
		if($this->request->isGet()){
			...
			if($this->request->isajax()){
				...
				$info['modellist'] = $this->model->ModelList();
				$info['page'] = $this->model->GetTemplate();
				//整理后的 所有的菜单
				$info['catelist'] = self::GetCateListData();
	...
	public function modeledit(){
		if($this->request->isGet()){
			...
			if($this->request->isajax()){
				...
				//获取所有模型
				$info['template'] = $this->model->GetTemplate();
				$info['authorlist'] = $this->model->AuthorList();
    ...
    #修改新闻状态#
    public function NewsStatusUp(){

    	if($this->request->isput()){

			$post=$this->request->put();

			$res = $this->model->ArticleStatusUp($post['set'],$post['newsid']);
		
			if($res){
				$this->success('新闻状态修改成功！');
			}
	...
	public function authorlist(){
		if($this->request->isGet()){
			...
			if($this->request->isAjax()){
                ...
                return  $NewsAuthor->getPage($param);
	...
	public function authoradd(){
		...
		if($this->request->isGet()){
            ...
            $res= $NewsAuthor->allowField(true)->save($post);
	...
	public function authoredit(){
		if($this->request->isGet()){
			...
			if($this->request->isajax()){
                ...
                return  NewsAuthor::get($authorid);
			}
	...
	public function authordel(){
		...
		if($this->request->isDelete()){
	          ...
	          $res = NewsAuthor::destroy($authorid);
	...
	public function comment(){
		if($this->request->isGet()){
            ...
            if($this->request->isAjax()){
                ...
                return  $NewsComment->getPage($param,'nickname|content|userid|touserid');
	...
	public function commentdel(){
		...
		if($this->request->isDelete()){
			...
			$commentid=$this->request->delete('commentid');

			$res = NewsComment::destroy($commentid);
